...
<style>
    ...
    /* BAYAR CARD */
    .bayar-card{background:var(--card-bg);border:1.5px solid var(--border);border-radius:14px;margin-bottom:16px;overflow:hidden;transition:box-shadow .2s;}
    .bayar-card:hover{box-shadow:0 4px 14px rgba(0,0,0,.07);}
    .bayar-card.new{border-color:var(--primary);}
    .bc-header{padding:12px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px;border-bottom:1px solid var(--border);background:#f8faff;}
    .bc-id{font-size:11.5px;font-weight:700;color:var(--muted);}
    .bc-body{padding:16px 18px;display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap;}
    .bc-av{width:46px;height:46px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:17px;color:#fff;flex-shrink:0;}
    .bc-info{flex:1;min-width:180px;}
    .bc-name{font-size:14px;font-weight:700;color:var(--text);}
    .bc-sub{font-size:12px;color:var(--muted);margin-top:3px;display:flex;align-items:center;gap:4px;}
    .bc-amount{font-size:18px;font-weight:800;color:var(--primary);flex-shrink:0;}
    .bukti-thumb{width:80px;height:60px;border-radius:8px;object-fit:cover;border:1.5px solid var(--border);cursor:pointer;transition:all .2s;background:var(--bg);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
    .bukti-thumb:hover{border-color:var(--primary);transform:scale(1.05);}
    .bc-actions{padding:12px 18px;background:var(--bg);border-top:1px solid var(--border);display:flex;gap:10px;flex-wrap:wrap;}
    .btn-aksi{border:none;border-radius:10px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s;flex:1;}
    ...
    .lightbox img{max-width:90%;max-height:88vh;border-radius:12px;object-fit:contain;box-shadow:0 20px 60px rgba(0,0,0,.5);}
    @media(max-width:767px){
        .bc-body{flex-direction:column;}
        .bc-actions{flex-direction:column;}
        .btn-aksi{width:100%;}
        .main-tabs{overflow-x:auto;flex-wrap:nowrap;}
        .main-tab{min-width:120px;flex:none;font-size:12px;}
        .tbl{font-size:11px;}
        .tbl td,.tbl th{padding:8px 10px!important;}
    }
</style>
@endpush
...
<div class="row g-3 mb-4">
    @php
    $stats = [
        ['bi-hourglass-split','var(--danger-soft)','var(--danger)','2','Menunggu Konfirmasi'],
        ['bi-check-circle-fill','var(--success-soft)','var(--success)','14','Dikonfirmasi'],
        ['bi-cash-coin','var(--success-soft)','var(--success)','Rp 1.2jt','Total Terkonfirmasi'],
        ['bi-x-circle-fill','var(--danger-soft)','var(--danger)','1','Ditolak'],
    ];
    @endphp
    @foreach($stats as $s)
...
<div id="tab-menunggu">
    ...
    $menungguList = [
        [
            ...
            'jadwal'   => 'Jumat, 2 Mei 2026 · 14:00 WIB',
            'durasi'   => '90 mnt · Online',
            'bank'     => 'Bank BCA',
            'norek'    => '1234567890',
            'jumlah'   => 'Rp 75.000',
            'masuk'    => '2 jam yang lalu',
        ],
        [
            'id'       => 'PAY-0051',
            'siswa'    => 'Siti Rahma',
            'email'    => 'siti@gmail.com',
            'mapel'    => 'Fisika – Gelombang',
            'jadwal'   => 'Sabtu, 3 Mei 2026 · 10:00 WIB',
            'durasi'   => '60 mnt · Offline',
            'bank'     => 'Bank BRI',
            'norek'    => '0987654321',
            'jumlah'   => 'Rp 50.000',
            'masuk'    => '5 jam yang lalu',
        ],
    ];
    @endphp
    ...
    <div class="bayar-card new">
        <div class="bc-header">
            ...
            <span style="background:var(--accent-soft);color:var(--warning);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">
                <i class="bi bi-hourglass-split me-1"></i>Menunggu · {{ $m['masuk'] }}
            </span>
        </div>
        <div class="bc-body">
            <div class="bc-av" style="background:var(--primary);">
                {{ strtoupper(substr($m['siswa'], 0, 2)) }}
            </div>
            <div class="bc-info">
                <div class="bc-name">{{ $m['siswa'] }}</div>
                <div class="bc-sub"><i class="bi bi-envelope"></i>{{ $m['email'] }}</div>
                <div class="bc-sub" style="margin-top:6px;"><i class="bi bi-book-fill"></i><strong>{{ $m['mapel'] }}</strong></div>
                <div class="bc-sub"><i class="bi bi-calendar3"></i>{{ $m['jadwal'] }} · {{ $m['durasi'] }}</div>
                <div class="bc-sub"><i class="bi bi-bank"></i>Transfer ke {{ $m['bank'] }} ({{ $m['norek'] }})</div>
                <div style="font-size:18px;font-weight:800;color:var(--primary);margin-top:8px;">{{ $m['jumlah'] }}</div>
            </div>

            {{-- BUKTI TRANSFER (placeholder) --}}
            <div>
                <div style="font-size:11px;color:var(--muted);margin-bottom:4px;font-weight:600;">Bukti Transfer:</div>
                <div class="bukti-thumb" onclick="openLightbox()">
                    <i class="bi bi-image" style="font-size:1.5rem;color:var(--muted);"></i>
                </div>
                <div style="font-size:10.5px;color:var(--primary);margin-top:4px;cursor:pointer;font-weight:600;" onclick="openLightbox()">
                    <i class="bi bi-zoom-in me-1"></i>Perbesar
                </div>
            </div>
        </div>
        <div class="bc-actions">
            <button class="btn-aksi" style="background:var(--danger-soft);color:var(--danger);"
                onclick="openModalTolak('{{ $m['id'] }}')">
                <i class="bi bi-x-circle me-1"></i> Tolak
            </button>
            <button class="btn-aksi" style="background:var(--primary);color:#fff;box-shadow:0 3px 10px rgba(30,58,95,.25);flex:2;"
                onclick="konfirmasiPembayaran('{{ $m['id'] }}','{{ $m['siswa'] }}')">
    ...
    <div class="card-box">
        ...
        <div style="overflow-x:auto;">
            <table class="tbl">
                ...
                @php
                $riwayatTutor = [
                    ['Aldi Pratama','Matematika – Trigonometri','Bank BCA','Rp 75.000','dikonfirmasi','29 Apr 2026 · 10:30'],
                    ['Sinta Dewi','Fisika – Gerak Parabola','Bank BRI','Rp 50.000','dikonfirmasi','22 Apr 2026 · 14:15'],
                    ['Maya Putri','Matematika – Integral','Bank BCA','Rp 100.000','dikonfirmasi','15 Apr 2026 · 09:00'],
                    ['Rizky Aditya','Kimia – Stoikiometri','Bank Mandiri','Rp 75.000','dikonfirmasi','8 Apr 2026 · 11:30'],
                    ['Farhan Maulana','B. Inggris – Grammar','Bank BCA','Rp 50.000','ditolak','1 Apr 2026 · 08:00'],
                    ['Aldi Pratama','Matematika – Kalkulus','Bank BCA','Rp 75.000','dikonfirmasi','25 Mar 2026 · 13:00'],
                ];
                @endphp
                @foreach($riwayatTutor as $r)
                <tr>
                    <td>
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div style="width:30px;height:30px;border-radius:50%;background:{{ $r[4]==='dikonfirmasi' ? 'var(--success)' : 'var(--muted)' }};color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;">{{ substr($r[0],0,1) }}</div>
                            <span style="font-weight:600;">{{ $r[0] }}</span>
                        </div>
                    </td>
                    <td>
                        <div style="font-weight:600;font-size:12.5px;">{{ explode('–',$r[1])[0] }}</div>
                        @if(isset(explode('–',$r[1])[1]))
                        <div style="font-size:11px;color:var(--muted);">– {{ trim(explode('–',$r[1])[1]) }}</div>
                        @endif
                    </td>
...
function tolakPembayaran() {
    ...
    // Simulasi FE only
    alert('Penolakan berhasil dicatat (simulasi FE).');
}

function konfirmasiPembayaran(id, siswa) {
    if(!confirm('Konfirmasi pembayaran dari ' + siswa + '?\nTindakan ini menandai sesi sebagai LUNAS.')) return;
    document.getElementById('sukses-pesan').innerHTML =
        'Pembayaran dari <strong>' + siswa + '</strong> berhasil dikonfirmasi. Status les privat otomatis menjadi <strong>Lunas</strong>.';
