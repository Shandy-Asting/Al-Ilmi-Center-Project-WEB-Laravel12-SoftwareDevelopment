...
@section('content')
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="stat-card">
                <div class="stat-icon" style="background:var(--danger-soft);color:var(--danger);"><i
        ...
        <div class="col-6 col-md-3">
            <div class="stat-card">
                <div class="stat-icon" style="background:#eff6ff;color:var(--primary);"><i class="bi bi-people-fill"></i>
                </div>
                <div>
    ...
    <div class="main-tabs">
        <button class="main-tab active" onclick="switchTab(this,'menunggu')">
            ...
            @if ($menunggu->count() > 0)
                <span
                    style="background:var(--danger);color:#fff;font-size:10px;padding:1px 6px;border-radius:20px;margin-left:4px;">{{ $menunggu->count() }}</span>
            @endif
        </button>
                ...
                <div class="rc-body">
                    ...
                    <div style="flex:1;">
                        ...
                        <div class="rc-sub mt-1">
                            ...
                            &nbsp;·&nbsp;
                            <span
                                style="background:{{ $m->mode === 'online' ? 'var(--success-soft)' : 'var(--info-soft)' }};color:{{ $m->mode === 'online' ? 'var(--success)' : 'var(--info)' }};font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:20px;">
                                {{ $m->getModeLabel() }}
                            </span>
                        </div>
                        @if ($m->catatan)
                    ...
                    <div style="text-align:right;flex-shrink:0;">
                        <div style="font-size:16px;font-weight:800;color:var(--primary);">Rp
        ...
        @forelse($dikonfirmasi as $d)
            <div class="request-card">
                ...
                <div class="rc-body">
                    ...
                    <div style="flex:1;">
                        ...
                        <div class="rc-sub mt-1">
                            ...
                            @if ($d->mode === 'online' && $d->link_meeting)
                                &nbsp;·&nbsp;
                                <a href="{{ $d->link_meeting }}" target="_blank"
                                    style="color:var(--primary);font-weight:600;font-size:11.5px;">
                    ...
                    <div style="text-align:right;flex-shrink:0;">
                        <div style="font-size:16px;font-weight:800;color:var(--primary);">Rp
                            {{ number_format($d->harga, 0, ',', '.') }}</div>
                        <div
                            style="font-size:11px;color:{{ $d->pembayaran_status === 'lunas' ? 'var(--success)' : 'var(--warning)' }};">
                                ...
                                <tr>
                                    <td>
                                        <div style="display:flex;align-items:center;gap:8px;">
                                            <div
                                                style="width:30px;height:30px;border-radius:50%;background:{{ $r->status === 'selesai' ? 'var(--success)' : 'var(--muted)' }};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0;">
                                    ...
                                    <td>
                                        <span
                                            style="background:#eff6ff;color:var(--primary);font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">{{ $r->mata_pelajaran }}</span>
                                        @if ($r->topik)
                                            <div style="font-size:11px;color:var(--muted);">{{ $r->topik }}</div>
                                        @endif
                                    </td>
                                    <td style="font-size:12.5px;color:var(--muted);">
                                        {{ $r->jadwal->format('d M Y · H:i') }} WIB</td>
                                    <td style="font-size:12.5px;">{{ $r->durasi_menit }} mnt</td>
                                    <td>
                                        <span
                                            style="background:{{ $r->mode === 'online' ? 'var(--success-soft)' : 'var(--info-soft)' }};color:{{ $r->mode === 'online' ? 'var(--success)' : 'var(--info)' }};font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">
                                            {{ $r->getModeLabel() }}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        @if ($r->status === 'selesai')
                                            <span
                                                style="background:var(--success-soft);color:var(--success);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">Selesai</span>
                                        @else
                                            <span
                                                style="background:var(--danger-soft);color:var(--danger);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">Dibatalkan</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
