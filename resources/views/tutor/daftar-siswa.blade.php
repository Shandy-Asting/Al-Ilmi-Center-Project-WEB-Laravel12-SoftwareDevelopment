...
<style>
    ...
    /* ── SISWA CARD ── */
    .siswa-card{background:var(--card-bg);border:1.5px solid var(--border);border-radius:16px;padding:18px;transition:all .2s;cursor:pointer;}
    .siswa-card:hover{border-color:var(--primary-light);box-shadow:0 6px 20px rgba(30,58,95,.1);transform:translateY(-2px);}
    .siswa-av{width:52px;height:52px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:20px;color:#fff;margin-bottom:12px;}
    .siswa-name{font-size:14px;font-weight:800;color:var(--text);margin-bottom:2px;}
    .siswa-info{font-size:12px;color:var(--muted);}
    .siswa-stats{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:12px;}
    .ss-item{background:var(--bg);border-radius:8px;padding:8px;text-align:center;}
    .ss-val{font-size:15px;font-weight:800;color:var(--primary);}
    .ss-label{font-size:10px;color:var(--muted);margin-top:2px;}
    .progress-bar-custom{height:6px;border-radius:10px;background:var(--border);margin-top:8px;overflow:hidden;}
    ...
    .vt-btn{width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:var(--bg);cursor:pointer;color:var(--muted);font-size:.9rem;border:none;transition:all .2s;}
    ...
    .modal-box{background:#fff;border-radius:20px;width:90%;max-width:540px;max-height:90vh;overflow-y:auto;animation:fadeUp .25s ease;}
...
{{-- FILTER BAR --}}
<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:14px;padding:14px 18px;display:flex;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:20px;">
    <div style="position:relative;flex:1;min-width:220px;">
        <i class="bi bi-search" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);"></i>
        <input type="text" style="width:100%;padding:9px 12px 9px 36px;border:1.5px solid var(--border);border-radius:10px;font-size:13px;outline:none;" placeholder="Cari nama siswa…" id="searchSiswa" oninput="filterSiswa()"/>
    </div>
    <select style="padding:9px 14px;border:1.5px solid var(--border);border-radius:10px;font-size:13px;outline:none;background:var(--card-bg);">
    ...
    <div class="view-toggle ms-auto">
        <button class="vt-btn active" id="btnGrid2" onclick="toggleView('grid')"><i class="bi bi-grid-3x3-gap-fill"></i></button>
...
<div id="view-grid">
    <div class="row g-3 mb-4">
        @php
        $siswas = [
            ['A','var(--primary)','Aldi Pratama','SMA · Kelas 12','Matematika','82','14','4','Bab 8/10'],
            ['S','var(--success)','Sinta Dewi','SMA · Kelas 11','Fisika','65','9','2','Bab 6/10'],
            ['R','var(--warning)','Rizky Aditya','SMA · Kelas 12','Kimia','50','11','3','Bab 5/10'],
            ['M','#6d28d9','Maya Putri','SMA · Kelas 12','Matematika','90','18','6','Bab 9/10'],
            ['F','var(--danger)','Farhan Maulana','SMP · Kelas 9','B. Inggris','40','7','1','Bab 4/10'],
            ['D','var(--info)','Dina Sari','SMA · Kelas 10','Biologi','75','12','4','Bab 7/10'],
            ['B','#0d9488','Budi Kurnia','SMP · Kelas 8','Matematika','68','8','2','Bab 6/10'],
            ['N','var(--accent)','Nadia Putri','SMA · Kelas 11','Kimia','88','16','5','Bab 8/10'],
        ];
        @endphp
        @foreach($siswas as $s)
        <div class="col-sm-6 col-lg-4 col-xl-3 siswa-card-wrap">
            <div class="siswa-card" onclick="openModalSiswa('{{ $s[2] }}')">
                <div class="siswa-av" style="background:{{ $s[1] }};">{{ $s[0] }}</div>
                <div class="siswa-name">{{ $s[2] }}</div>
                <div class="siswa-info">{{ $s[3] }}</div>
                <div class="siswa-info" style="margin-top:3px;">
                    <span style="background:#eff6ff;color:var(--primary);font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:20px;">{{ $s[4] }}</span>
                </div>
                <div class="siswa-stats">
                    <div class="ss-item">
                        <div class="ss-val" style="color:{{ $s[5] >= 70 ? 'var(--success)' : ($s[5] >= 60 ? 'var(--warning)' : 'var(--danger)') }};">{{ $s[5] }}</div>
                        <div class="ss-label">Rata-rata</div>
                    </div>
                    <div class="ss-item">
                        <div class="ss-val">{{ $s[6] }}</div>
                        <div class="ss-label">Sesi Total</div>
                    </div>
                </div>
                <div class="mt-2">
                    <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px;">
                        <span style="color:var(--muted);">Progres</span>
                        <span style="color:var(--primary);font-weight:700;">{{ $s[8] }}</span>
                    </div>
                    <div class="progress-bar-custom">
                        <div class="pb-fill" style="width:{{ $s[5] }}%;background:{{ $s[5] >= 70 ? 'var(--success)' : ($s[5] >= 60 ? 'var(--warning)' : 'var(--danger)') }};"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card-box mb-4">
        ...
        <div style="overflow-x:auto;">
            <table class="tbl">
                ...
                @php
                $siswasTable = [
                    ['A','var(--primary)','Aldi Pratama','SMA 12','Matematika','82','14','Bab 8/10','baik'],
                    ['S','var(--success)','Sinta Dewi','SMA 11','Fisika','65','9','Bab 6/10','cukup'],
                    ['R','var(--warning)','Rizky Aditya','SMA 12','Kimia','50','11','Bab 5/10','perhatian'],
                    ['M','#6d28d9','Maya Putri','SMA 12','Matematika','90','18','Bab 9/10','baik'],
                    ['F','var(--danger)','Farhan Maulana','SMP 9','B. Inggris','40','7','Bab 4/10','perhatian'],
                ...
                <tr>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div style="width:34px;height:34px;border-radius:50%;background:{{ $st[1] }};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;flex-shrink:0;">{{ $st[0] }}</div>
                            <div>
                    ...
                    <td><span style="background:#eff6ff;color:var(--primary);font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">{{ $st[4] }}</span></td>
                    <td>
                        <span style="font-size:15px;font-weight:800;color:{{ $st[5] >= 70 ? 'var(--success)' : ($st[5] >= 60 ? 'var(--warning)' : 'var(--danger)') }};">{{ $st[5] }}</span>
                    </td>
                    <td style="font-size:12.5px;">{{ $st[6] }} sesi</td>
                    <td>
                        <div style="font-size:11.5px;color:var(--muted);margin-bottom:4px;">{{ $st[7] }}</div>
                        <div style="height:5px;border-radius:10px;background:var(--border);width:80px;overflow:hidden;">
                            <div style="height:100%;border-radius:10px;width:{{ $st[5] }}%;background:{{ $st[5] >= 70 ? 'var(--success)' : ($st[5] >= 60 ? 'var(--warning)' : 'var(--danger)') }};"></div>
                        </div>
                    </td>
                    <td>
                        ...
                        <span style="background:{{ $stLabel[$st[8]][0] }};color:{{ $stLabel[$st[8]][1] }};font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">{{ $stLabel[$st[8]][2] }}</span>
                    </td>
...
<div class="modal-overlay" id="modal-siswa">
    <div class="modal-box">
        ...
        <div style="padding:20px 22px;">
            {{-- Profile --}}
            <div style="display:flex;align-items:center;gap:14px;padding:14px;background:var(--bg);border-radius:12px;margin-bottom:16px;">
                <div style="width:56px;height:56px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:22px;color:#fff;flex-shrink:0;">A</div>
                <div style="flex:1;">
                    <div style="font-size:15px;font-weight:800;color:var(--text);" id="modal-siswa-name">Aldi Pratama</div>
                    <div style="font-size:12px;color:var(--muted);">SMA Kelas 12 · Matematika</div>
                    <div style="margin-top:6px;display:flex;gap:6px;">
                        <span style="background:var(--success-soft);color:var(--success);font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:20px;">Aktif</span>
                        <span style="background:#eff6ff;color:var(--primary);font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:20px;">14 Sesi</span>
                    </div>
                </div>
            ...
            <div style="margin-bottom:8px;">
                <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;">
                    ...
                    <span style="font-weight:700;color:{{ $t[2] }};">{{ $t[1] === 'Sedang' ? '🔵 Sedang' : $t[1].'%' }}</span>
                </div>
                @if($t[1] !== 'Sedang')
                <div style="height:6px;border-radius:10px;background:var(--border);overflow:hidden;">
                    <div style="height:100%;border-radius:10px;width:{{ $t[1] }}%;background:{{ $t[2] }};"></div>
                </div>
                @endif
            </div>
            @endforeach
            ...
            <div class="d-flex gap-2 mt-4">
                <button style="flex:1;padding:10px;border-radius:10px;border:1.5px solid var(--border);background:var(--bg);color:var(--muted);font-size:13px;font-weight:700;cursor:pointer;"
                    onclick="closeModal()">Tutup</button>
                <a href="/tutor/jadwal" style="flex:1;padding:10px;border-radius:10px;border:none;background:var(--primary);color:#fff;font-size:13px;font-weight:700;cursor:pointer;text-align:center;text-decoration:none;">
