...
@section('content')
    ...
    <div class="main-tabs">
        ...
        <button class="main-tab" onclick="switchTab(this,'permintaan')">
            ...
            <span
                style="background:var(--danger);color:#fff;font-size:10px;padding:1px 5px;border-radius:20px;margin-left:4px;">3</span>
        </button>
    ...
    <div id="tab-hari-ini">
        <div class="row g-3">
                ...
                <div class="card-box">
                    <div class="card-box-header">
                        <div class="card-box-title">
                            ...
                            <span style="font-size:12px;font-weight:500;color:var(--muted);margin-left:4px;">Rabu, 29 April
                                2026</span>
                        </div>
                        <span
                        ...
                        <div class="jadwal-item {{ $j[7] === 'sedang' ? 'sedang' : '' }}">
                            ...
                            <div class="sched-info">
                                <div class="sched-subject">
                                    {{ $j[2] }}
                                    <span
                                        style="background:#eff6ff;color:var(--primary);font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:20px;margin-left:6px;">{{ $j[3] }}</span>
                                    @if ($j[7] === 'sedang')
                                        <span
                                            style="background:var(--danger-soft);color:var(--danger);font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:20px;margin-left:4px;">🔴
                                            Sedang Berlangsung</span>
                                    @elseif($j[7] === 'selesai')
                                        <span
                                            style="background:var(--success-soft);color:var(--success);font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:20px;margin-left:4px;">✅
                                            Selesai</span>
                                    @endif
                                </div>
                                <div class="sched-student">
                                    ...
                                    <i
                                        class="bi bi-{{ $j[6] === 'online' ? 'camera-video-fill' : 'geo-alt-fill' }} me-1"></i>{{ $j[5] }}
                                </div>
                            </div>
                            <span class="sched-mode {{ $j[6] }}">{{ ucfirst($j[6]) }}</span>
                            <div class="d-flex gap-1">
                                <button class="btn-sched" title="Detail" onclick="openModalDetail()"><i
                                        class="bi bi-eye"></i></button>
                                @if ($j[7] === 'akan-datang' || $j[7] === 'sedang')
                                    <button class="btn-sched" title="Mulai/Lanjut"
                                        style="background:var(--primary);color:#fff;border-color:var(--primary);"><i
                                            class="bi bi-play-fill"></i></button>
                                @endif
                                @if ($j[7] === 'selesai')
                                    <button class="btn-sched" title="Beri Catatan"><i
            ...
            <div class="col-lg-4">
                <div class="card-box mb-3" style="overflow:visible;">
                    ...
                    <div class="p-3">
                        ...
                        @foreach ($ringkasan as $r)
                            <div
                                style="display:flex;align-items:center;gap:10px;padding:10px;background:{{ $r[2] }};border-radius:10px;margin-bottom:8px;">
                                <i class="bi {{ $r[4] }}"
                                    style="color:{{ $r[3] }};font-size:18px;width:24px;flex-shrink:0;"></i>
                                <div style="flex:1;">
                                    <div style="font-size:12px;color:var(--muted);">{{ $r[0] }}</div>
                                </div>
                                <div style="font-size:15px;font-weight:800;color:{{ $r[3] }};">
                                    {{ $r[1] }}</div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div
                    style="background:linear-gradient(135deg,var(--primary),var(--primary-light));border-radius:16px;padding:18px;color:#fff;">
                    <div
                        style="font-size:11px;font-weight:700;opacity:.75;margin-bottom:6px;text-transform:uppercase;letter-spacing:.06em;">
                        Sesi Berikutnya</div>
                    <div style="font-size:16px;font-weight:800;margin-bottom:4px;">Matematika – Integral</div>
                    <div style="font-size:12px;opacity:.75;margin-bottom:12px;"><i class="bi bi-person-fill me-1"></i>
                        Maya Putri · 15:30 WIB</div>
            ...
            <div class="col-lg-8">
                <div class="cal-wrap">
                    ...
                    <div style="display:flex;gap:12px;margin-top:14px;font-size:11.5px;color:var(--muted);">
                        <span><span
                                style="width:10px;height:10px;border-radius:50%;background:var(--primary);display:inline-block;margin-right:4px;"></span>
                            Hari Ini</span>
                        <span><span
                                style="width:8px;height:8px;border-radius:50%;background:var(--accent);display:inline-block;margin-right:4px;"></span>
                            Ada Sesi</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                <div class="card-box">
                    <div class="card-box-header">
                        ...
                        <span
                            style="background:var(--success-soft);color:var(--success);font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;">5
                            Sesi</span>
                    </div>
                    @foreach ([['08.00', 'Matematika', 'Aldi', 'online'], ['10.30', 'Fisika', 'Sinta', 'online'], ['13.00', 'Kimia', 'Rizky', 'offline'], ['15.30', 'Matematika', 'Maya', 'online'], ['17.00', 'B. Inggris', 'Farhan', 'online']] as $j)
                        <div
                            style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-bottom:1px solid var(--border);">
                            ...
                            <span
                                style="background:{{ $j[3] === 'online' ? 'var(--success-soft)' : '#f5f3ff' }};color:{{ $j[3] === 'online' ? 'var(--success)' : '#6d28d9' }};font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;">{{ ucfirst($j[3]) }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="row g-3">
            <div class="col-lg-8">
                <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:14px;">
                    🔔 Permintaan Masuk <span style="font-size:12px;color:var(--muted);font-weight:500;">(3 permintaan
                        baru)</span>
                </div>
                        ...
                        <div class="rc-body">
                            ...
                            <div style="flex:1;">
                                ...
                                <div class="rc-sub mt-1">
                                    ...
                                    &nbsp;·&nbsp;
                                    <span
                                        style="background:{{ $r[7] === 'Online' ? 'var(--success-soft)' : 'var(--info-soft)' }};color:{{ $r[7] === 'Online' ? 'var(--success)' : 'var(--info)' }};font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:20px;">{{ $r[7] }}</span>
                                </div>
                            </div>
            ...
            <div class="col-lg-4">
                <div class="card-box p-3">
                    <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:12px;">📊 Statistik
                        Permintaan</div>
                    @php
                        $reqStats = [
                    ...
                    @foreach ($reqStats as $rs)
                        <div
                            ...
                            <span
                                style="font-size:16px;font-weight:800;color:{{ $rs[2] }};">{{ $rs[1] }}</span>
                        </div>
                    @endforeach
                    <div
                        style="margin-top:12px;padding:10px;background:var(--success-soft);border-radius:10px;text-align:center;">
    ...
    <div id="tab-semua" style="display:none;">
        <div class="d-flex gap-2 mb-3 flex-wrap">
            <div style="position:relative;flex:1;min-width:200px;">
                ...
                <input type="text"
                    style="border:1.5px solid var(--border);border-radius:10px;padding:9px 12px 9px 36px;font-size:13px;width:100%;outline:none;"
                    placeholder="Cari nama siswa, mata pelajaran…" />
            </div>
            <select
                style="border:1.5px solid var(--border);border-radius:10px;padding:9px 14px;font-size:13px;outline:none;background:var(--card-bg);">
                    ...
                    <tbody>
                        ...
                        @foreach ($semuaJadwal as $sj)
                            <tr onmouseover="this.style.background='#f8faff'" onmouseout="this.style.background=''">
                                <td
                                    style="padding:12px 14px;border-bottom:1px solid var(--border);font-size:12.5px;font-weight:700;color:var(--primary);">
                                    {{ $sj[0] }}</td>
                                <td
                                    style="padding:12px 14px;border-bottom:1px solid var(--border);font-size:13px;font-weight:600;">
                                    {{ $sj[1] }}</td>
                                <td style="padding:12px 14px;border-bottom:1px solid var(--border);">
                                    <span
                                        style="background:#eff6ff;color:var(--primary);font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">{{ $sj[2] }}</span>
                                </td>
                                <td
                                    style="padding:12px 14px;border-bottom:1px solid var(--border);font-size:12.5px;color:var(--muted);">
                                    {{ $sj[3] }}</td>
                                <td style="padding:12px 14px;border-bottom:1px solid var(--border);">
                                    <span
                                        style="background:{{ $sj[4] === 'Online' ? 'var(--success-soft)' : 'var(--info-soft)' }};color:{{ $sj[4] === 'Online' ? 'var(--success)' : 'var(--info)' }};font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">{{ $sj[4] }}</span>
                                </td>
                                <td style="padding:12px 14px;border-bottom:1px solid var(--border);">
                                    @php
                                        $stColors = [
                                    ...
                                    @endphp
                                    <span
                                        style="background:{{ $sc[0] }};color:{{ $sc[1] }};font-size:11px;font-weight:700;padding:3px 8px;border-radius:20px;">{{ $sc[2] }}</span>
                                </td>
                                <td style="padding:12px 14px;border-bottom:1px solid var(--border);">
