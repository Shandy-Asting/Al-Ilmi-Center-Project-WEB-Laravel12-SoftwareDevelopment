...
@section('content')
    ...
    @if ($errors->any())
        <div
            style="background:var(--danger-soft);border:1px solid var(--danger);border-radius:12px;padding:12px 16px;margin-bottom:16px;">
            @foreach ($errors->all() as $error)
                <div style="font-size:13px;color:var(--danger);"><i
            ...
            <div class="row g-3">
                <div class="col-lg-8">
                    ...
                    @forelse($tutors as $tutor)
                        <div class="tutor-card" onclick="selectTutor(this)" data-id="{{ $tutor->id }}"
                ...
                <div class="col-lg-4">
                    <div class="summary-card">
                        ...
                        <div class="summary-row"><span class="sr-label">Layanan</span><span class="sr-val"
                                id="sum-layanan">-</span></div>
                        <div class="summary-row"><span class="sr-label">Tutor</span><span class="sr-val"
                                id="sum-tutor">Belum dipilih</span></div>
                ...
                <div class="row g-3">
                        ...
                        <div class="form-card mb-3">
                            ...
                            <div style="background:var(--bg);border-radius:12px;padding:16px;margin-bottom:16px;">
                                <div class="d-flex align-items-center gap-3">
                                    <div style="width:52px;height:52px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:18px;color:#fff;flex-shrink:0;"
                                        id="konfirm-avatar">?</div>
                                    <div>
                                        <div style="font-size:14px;font-weight:800;color:var(--text);"
                                            id="konfirm-tutor-nama">-</div>
                                        <div style="font-size:12px;color:var(--muted);">Tutor Al Ilmi Center</div>
                                    </div>
                                </div>
                                <div class="mt-3 d-flex flex-column gap-2">
                                    <div
                                        style="display:flex;justify-content:space-between;font-size:13px;padding:6px 0;border-bottom:1px solid var(--border);">
                            ...
                            <div class="row g-3 mb-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="mata_pelajaran" class="form-control-custom" id="sel-mapel"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="topik" class="form-control-custom"
                                        placeholder="Contoh: Integral, Trigonometri…" />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label-custom">Tanggal & Waktu <span
                                            class="text-danger">*</span></label>
                                    <input type="datetime-local" name="jadwal" id="input-jadwal"
                                        class="form-control-custom" min="{{ now()->addDay()->format('Y-m-d\TH:i') }}"
                                        onchange="updateKonfirm()" />
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label-custom">Durasi</label>
                                    <select name="durasi_menit" class="form-control-custom" id="sel-durasi"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="mode" class="form-control-custom" id="sel-mode"
                                        onchange="updateMode()">
                    ...
                    <div class="col-lg-4">
                        <div class="summary-card">
                            ...
                            <div class="summary-row"><span class="sr-label">Biaya Sesi</span><span class="sr-val"
                                    id="biaya-sesi">Rp 75.000</span></div>
                            <div class="summary-row"><span class="sr-label">Biaya Admin</span><span class="sr-val">Rp
                                    2.000</span></div>
    ...
    <div id="panel-status" style="display:none;">
            ...
            <div class="order-card" style="{{ $p->status === 'menunggu' ? 'border-color:var(--accent);' : '' }}">
                <div class="order-header">
                    ...
                    @if ($p->status === 'menunggu')
                        <span class="pill p-warning" style="font-size:11px;"><i class="bi bi-hourglass-split"></i>
                ...
                <div class="order-body">
                    ...
                    <div class="order-action">
                        <div style="text-align:right;">
                            <div style="font-size:14px;font-weight:800;color:var(--primary);">Rp
                                {{ number_format($p->harga, 0, ',', '.') }}</div>
                            <div style="font-size:11px;color:var(--muted);">
                                {{ $p->pembayaran_status === 'lunas' ? '✅ Lunas' : '⏳ Belum Bayar' }}</div>
                        </div>
                        @if ($p->status === 'menunggu')
        ...
        @empty
            <div
                style="text-align:center;padding:40px;color:var(--muted);background:var(--card-bg);border-radius:16px;border:1px solid var(--border);">
                <i class="bi bi-calendar-x" style="font-size:2.5rem;margin-bottom:10px;display:block;"></i>
                Belum ada pesanan aktif.
                <div style="margin-top:10px;">
                    <button onclick="showTab('pesan')"
                        style="background:var(--primary);color:#fff;border:none;border-radius:10px;padding:8px 20px;font-size:13px;font-weight:700;cursor:pointer;">
                        Pesan Les Sekarang
                    </button>
                </div>
            </div>
            ...
            @foreach ($riwayat as $r)
                <div class="order-card" style="opacity:.85;">
                    <div class="order-header">
                        ...
                        @if ($r->status === 'selesai')
                            <span class="pill p-success" style="font-size:11px;"><i class="bi bi-check2-all"></i>
                                Selesai</span>
                        @else
                            <span class="pill p-danger" style="font-size:11px;"><i class="bi bi-x-circle-fill"></i>
                                Dibatalkan</span>
                        @endif
                    </div>
                    <div class="order-body">
                        <div class="order-av"
                            style="background:{{ $r->status === 'selesai' ? 'var(--success)' : 'var(--muted)' }};">
                            {{ strtoupper(substr($r->tutor->name ?? 'T', 0, 2)) }}
                        </div>
                        <div class="order-info">
                            <div class="oi-subj">{{ $r->mata_pelajaran }}{{ $r->topik ? ' – ' . $r->topik : '' }}</div>
                            <div class="oi-tutor"><i class="bi bi-person-fill me-1"></i> {{ $r->tutor->name ?? '-' }}
                            </div>
                            <div class="oi-sched">
                                <i class="bi bi-calendar3 me-1"></i>
                        ...
                        <div class="order-action">
                            <div style="font-size:14px;font-weight:800;color:var(--muted);">Rp
                                {{ number_format($r->harga, 0, ',', '.') }}</div>
                            @if ($r->status === 'selesai')
                                <button onclick="showTab('pesan')" class="btn-action"
                                    style="background:#eff6ff;color:var(--primary);margin-top:6px;">Pesan Ulang</button>
                            @endif
                        </div>
                    </div>
                </div>
