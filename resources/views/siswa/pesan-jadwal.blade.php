...
<style>
    ...
    .step-circle{width:36px;height:36px;border-radius:50%;border:2px solid var(--border);background:var(--card-bg);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:var(--muted);position:relative;z-index:1;transition:all .3s;}
    .step-item.active .step-circle{background:var(--primary);border-color:var(--primary);color:#fff;box-shadow:0 4px 12px rgba(30,58,95,.3);}
    .step-item.done .step-circle{background:var(--success);border-color:var(--success);color:#fff;}
    .step-label{font-size:11.5px;font-weight:600;color:var(--muted);margin-top:6px;text-align:center;}
    .step-item.active .step-label{color:var(--primary);font-weight:700;}
    .step-item.done .step-label{color:var(--success);}

    /* ── TUTOR CARDS ── */
    .tutor-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;margin-top:12px;}
    .tutor-card{background:var(--card-bg);border:2px solid var(--border);border-radius:14px;padding:16px;text-align:center;cursor:pointer;transition:all .2s;}
    .tutor-card:hover{border-color:var(--primary-light);transform:translateY(-2px);box-shadow:0 6px 20px rgba(30,58,95,.1);}
    .tutor-card.selected{border-color:var(--primary);background:#eff6ff;box-shadow:0 6px 20px rgba(30,58,95,.15);}
    .tutor-avatar{width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:20px;margin:0 auto 10px;}
    .tutor-name{font-size:13.5px;font-weight:700;color:var(--text);margin-bottom:2px;}
    .tutor-mapel{font-size:11.5px;color:var(--muted);}
    .tutor-rating{font-size:12px;color:var(--warning);margin-top:4px;font-weight:600;}
    .tutor-badge{font-size:10px;padding:2px 8px;border-radius:20px;margin-top:6px;display:inline-block;}

    /* ── JADWAL SLOT ── */
    .slot-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:8px;margin-top:10px;}
    .slot-btn{padding:10px 8px;border:2px solid var(--border);border-radius:10px;font-size:12px;font-weight:600;color:var(--muted);cursor:pointer;transition:all .2s;background:var(--card-bg);text-align:center;}
    .slot-btn:hover{border-color:var(--primary-light);color:var(--primary);}
    .slot-btn.selected{border-color:var(--primary);background:#eff6ff;color:var(--primary);}
    .slot-btn.taken{background:var(--bg);color:var(--border);cursor:not-allowed;border-color:var(--border);}

    /* ── FORM SECTION ── */
    .form-section{background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:24px;margin-bottom:16px;}
    .form-section-title{font-size:15px;font-weight:700;color:var(--text);margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;}
    ...
    /* ── SUMMARY CARD ── */
    .summary-card{background:linear-gradient(135deg,var(--primary) 0%,var(--primary-light) 100%);border-radius:16px;padding:20px;color:#fff;margin-bottom:16px;}
    .sc-row{display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.12);}
    .sc-row:last-child{border-bottom:none;}
    .sc-label{font-size:12.5px;opacity:.75;}
    ...
    /* ── STATUS BADGE ── */
    .status-badge{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:20px;font-size:11.5px;font-weight:700;}
    ...
    .modal-close-btn{width:32px;height:32px;border-radius:8px;border:none;background:var(--bg);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:16px;color:var(--muted);}
    ...
    .card-box-title{font-size:14.5px;font-weight:700;color:var(--text);}
</style>
@endpush
...
<div id="tab-pesan">
    ...
    <div id="panel-1" class="form-section">
        ...
        <div class="row g-2 mb-3">
            <div class="col-md-4">
                <div style="position:relative;">
                    ...
                    <input type="text" class="form-control-custom" style="padding-left:36px;" placeholder="Cari nama tutor…" id="searchTutor" oninput="filterTutor()"/>
                </div>
            </div>
            <div class="col-md-4">
        ...
        <div class="tutor-grid" id="tutorGrid">
            @php
            $tutors = [
                ['B','#1e3a5f','#eff6ff','Pak Budi Santoso','Matematika','4.9',42,'bg-primary','Online & Offline','Rp 75rb/sesi'],
                ['S','#065f46','#dcfce7','Bu Sari Dewi','Fisika','4.8',38,'bg-success','Online & Offline','Rp 90rb/sesi'],
                ['R','#7c3aed','#f5f3ff','Pak Rizal Hakim','Kimia','4.7',29,'bg-purple','Online','Rp 80rb/sesi'],
                ['A','#d97706','#fef3c7','Bu Anisa Putri','Biologi','4.9',51,'bg-warning','Online & Offline','Rp 85rb/sesi'],
                ['F','#0284c7','#e0f2fe','Pak Fauzan','B. Inggris','4.6',25,'bg-info','Online','Rp 70rb/sesi'],
                ['D','#dc2626','#fee2e2','Bu Dita Rahma','Matematika','4.8',33,'bg-danger','Offline','Rp 75rb/sesi'],
            ];
            @endphp
            @foreach($tutors as $idx => $t)
            <div class="tutor-card" onclick="selectTutor(this, {{ $idx }})"
                data-name="{{ $t[3] }}" data-mapel="{{ $t[4] }}"
                data-harga="{{ $t[9] }}" data-mode="{{ $t[7] }}">
                <div class="tutor-avatar" style="background:{{ $t[1] }};color:#fff;">{{ $t[0] }}</div>
                <div class="tutor-name">{{ $t[3] }}</div>
                <div class="tutor-mapel">{{ $t[4] }}</div>
                <div class="tutor-rating">
                    <i class="bi bi-star-fill" style="color:var(--accent);"></i> {{ $t[5] }}
                    <span style="color:var(--muted);font-weight:400;">({{ $t[6] }} ulasan)</span>
                </div>
                <div class="tutor-badge" style="background:{{ str_contains($t[7],'Online & Offline') ? 'var(--success-soft)' : 'var(--info-soft)' }};color:{{ str_contains($t[7],'Online & Offline') ? 'var(--success)' : 'var(--info)' }};">
                    {{ $t[7] }}
                </div>
                <div style="font-size:12px;font-weight:700;color:var(--primary);margin-top:6px;">{{ $t[9] }}</div>
            </div>
            @endforeach
        </div>
    ...
    <div id="panel-2" style="display:none;" class="form-section">
        ...
        <div class="mt-3">
            ...
            <div class="slot-grid" id="slotGrid">
                @php
                $slots = ['07:00','08:00','09:00','10:00','11:00','13:00','14:00','15:00','16:00','17:00','19:00','20:00'];
        ...
        <div class="row g-3">
                ...
                <div class="form-section">
                    ...
                    <div style="background:var(--bg);border-radius:12px;padding:16px;margin-bottom:16px;">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <div style="width:48px;height:48px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;color:#fff;font-size:20px;font-weight:800;">
                                <span id="konfirm-avatar">B</span>
                            </div>
                            <div>
                                <div style="font-size:15px;font-weight:700;color:var(--text);" id="konfirm-tutor">Pak Budi Santoso</div>
                        ...
                        <div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
                            <i class="bi {{ $kr[0] }}" style="color:var(--primary);font-size:15px;width:20px;flex-shrink:0;margin-top:1px;"></i>
                            <div style="flex:1;">
                                <span style="font-size:12px;color:var(--muted);">{{ $kr[1] }}</span>
                                <div style="font-size:13px;font-weight:600;color:var(--text);" id="{{ $kr[2] }}">{{ $kr[3] }}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    ...
                    {{-- Metode Pembayaran --}}
                    <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:10px;">Metode Pembayaran</div>
                    @php
                    $payMethods = [
                    ...
                    <div style="background:var(--card-bg);border:2px solid {{ $pm[5] ? 'var(--primary)' : 'var(--border)' }};border-radius:12px;padding:12px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;margin-bottom:8px;transition:all .2s;{{ $pm[5] ? 'background:#eff6ff;' : '' }}"
                        ...
                        <div style="width:44px;height:28px;border-radius:6px;background:{{ $pm[0] }};color:{{ $pm[1] }};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;flex-shrink:0;">{{ $pm[2] }}</div>
                        <div style="flex:1;">
                            <div style="font-size:13px;font-weight:700;color:var(--text);">{{ $pm[3] }}</div>
                            <div style="font-size:11.5px;color:var(--muted);">{{ $pm[4] }}</div>
                        </div>
                        <div style="width:18px;height:18px;border-radius:50%;border:2px solid {{ $pm[5] ? 'var(--primary)' : 'var(--border)' }};background:{{ $pm[5] ? 'var(--primary)' : 'transparent' }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            @if($pm[5])<i class="bi bi-check-lg" style="font-size:10px;color:#fff;"></i>@endif
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="col-lg-5">
                ...
                <div class="form-section">
                    ...
                    @php
                    $jaminans = [
                        ['bi-shield-fill-check','var(--success)','Pembayaran Aman','Transaksi terenkripsi SSL'],
                        ['bi-arrow-repeat','var(--primary)','Reschedule Gratis','Dapat diubah 24 jam sebelum'],
                        ['bi-star-fill','var(--accent)','Tutor Terverifikasi','Semua tutor sudah terverifikasi'],
                        ['bi-headset','var(--info)','Dukungan 24/7','CS siap membantu kapanpun'],
                    ];
                    @endphp
                    @foreach($jaminans as $j)
                    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
                        <i class="bi {{ $j[0] }}" style="color:{{ $j[1] }};font-size:16px;width:20px;flex-shrink:0;"></i>
                        <div>
                            <div style="font-size:12.5px;font-weight:700;color:var(--text);">{{ $j[1] }}</div>
...
<div id="tab-riwayat" style="display:none;">
    ...
    <div class="d-flex gap-2 mb-3 flex-wrap align-items-center">
        <div style="position:relative;flex:1;min-width:200px;">
            <i class="bi bi-search" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:14px;"></i>
            <input type="text" class="form-control-custom" style="padding-left:36px;" placeholder="Cari nama tutor, mata pelajaran…"/>
        </div>
    ...
    <div class="card-box">
        ...
        <div style="overflow-x:auto;">
            <table class="tbl" id="tabelRiwayat">
                ...
                @php
                $pesanans = [
                    ['#PES-2026-0412','Pak Budi Santoso','Matematika','8 Apr 2026, 10:00','90 mnt','Online','Rp 75.000','dikonfirmasi','var(--success-soft)','var(--success)'],
                    ['#PES-2026-0398','Bu Sari Dewi','Fisika','5 Apr 2026, 14:00','90 mnt','Offline','Rp 90.000','selesai','#eff6ff','var(--primary)'],
                    ['#PES-2026-0385','Pak Rizal Hakim','Kimia','2 Apr 2026, 09:00','60 mnt','Online','Rp 80.000','selesai','#eff6ff','var(--primary)'],
                    ['#PES-2026-0371','Bu Anisa Putri','Biologi','28 Mar 2026, 11:00','90 mnt','Offline','Rp 85.000','selesai','#eff6ff','var(--primary)'],
                    ['#PES-2026-0358','Pak Budi Santoso','Matematika','25 Mar 2026, 10:00','120 mnt','Online','Rp 150.000','selesai','#eff6ff','var(--primary)'],
                    ['#PES-2026-0340','Pak Fauzan','B. Inggris','20 Mar 2026, 16:00','60 mnt','Online','Rp 70.000','dibatalkan','var(--danger-soft)','var(--danger)'],
                ];
                @endphp
                ...
                <tr>
                    ...
                    <td>
                        <span style="background:#eff6ff;color:var(--primary);font-size:11px;font-weight:600;padding:3px 8px;border-radius:20px;">{{ $p[2] }}</span>
                    </td>
                    <td style="font-size:12.5px;color:var(--muted);">{{ $p[3] }}</td>
                    <td style="font-size:12.5px;">{{ $p[4] }}</td>
                    <td>
                        <span style="background:{{ $p[5] === 'Online' ? 'var(--info-soft)' : 'var(--success-soft)' }};color:{{ $p[5] === 'Online' ? 'var(--info)' : 'var(--success)' }};font-size:11px;font-weight:600;padding:3px 8px;border-radius:20px;">
                            {{ $p[5] }}
                        </span>
                    </td>
                    <td style="font-weight:700;">{{ $p[6] }}</td>
                    <td>
                        <span class="status-badge" style="background:{{ $p[8] }};color:{{ $p[9] }};">
                            {{ ucfirst($p[7]) }}
                        </span>
                    </td>
                    <td style="text-align:center;">
                        <div class="d-flex gap-1 justify-content-center">
                            ...
                            @if($p[7] === 'dikonfirmasi')
                            <button style="border:none;background:var(--danger-soft);color:var(--danger);border-radius:8px;padding:5px 10px;font-size:11.5px;font-weight:700;cursor:pointer;">
                                Batal
                            </button>
                            @endif
                            @if($p[7] === 'selesai')
                            <button style="border:none;background:var(--accent-soft);color:var(--warning);border-radius:8px;padding:5px 10px;font-size:11.5px;font-weight:700;cursor:pointer;">
                                Ulasan
                            </button>
                            @endif
                        </div>
...
<div class="modal-overlay" id="modal-sukses">
    <div class="modal-box">
        <div class="modal-body-p" style="text-align:center;padding:36px 28px;">
            ...
            <div style="background:var(--success-soft);border-radius:12px;padding:16px;margin-bottom:20px;text-align:left;">
...
<div class="modal-overlay" id="modal-detail">
    <div class="modal-box">
        ...
        <div class="modal-body-p">
            <div style="background:var(--bg);border-radius:12px;padding:16px;margin-bottom:16px;">
                <div style="font-size:13px;font-weight:700;color:var(--primary);margin-bottom:10px;" id="detail-id">#PES-2026-0412</div>
                @php
                $detailRows = [
            ...
            <div class="d-flex gap-2">
                ...
                <button onclick="exportSinglePDF()"
                    class="btn fw-bold flex-fill py-2"
...
function selectTutor(el, idx) {
    ...
    selectedTutor = {
        ...
        avatar: el.querySelector('.tutor-name').textContent.trim()[0]
    };
}
...
function updateSlots() { /* bisa dikembangkan untuk fetch slot dari server */ }
...
function updateKonfirmasi() {
    ...
    if(tgl && selectedSlot){
        const fmt = new Date(tgl).toLocaleDateString('id-ID',{weekday:'long',day:'numeric',month:'short',year:'numeric'});
    ...
    if(mode) document.getElementById('konfirm-mode').textContent = mode === 'online' ? 'Online (Zoom/Google Meet)' : 'Offline (Tatap Muka)';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(modeSelect){
        modeSelect.addEventListener('change', function(){
            document.getElementById('alamatWrap').style.display = this.value === 'offline' ? '' : 'none';
        });
    }
});
...
function exportExcel() {
    const rows = [
        ...
        ['#PES-2026-0412','Pak Budi Santoso','Matematika','8 Apr 2026, 10:00','90 mnt','Online','Rp 75.000','Dikonfirmasi'],
        ['#PES-2026-0398','Bu Sari Dewi','Fisika','5 Apr 2026, 14:00','90 mnt','Offline','Rp 90.000','Selesai'],
        ['#PES-2026-0385','Pak Rizal Hakim','Kimia','2 Apr 2026, 09:00','60 mnt','Online','Rp 80.000','Selesai'],
        ['#PES-2026-0371','Bu Anisa Putri','Biologi','28 Mar 2026, 11:00','90 mnt','Offline','Rp 85.000','Selesai'],
        ['#PES-2026-0358','Pak Budi Santoso','Matematika','25 Mar 2026, 10:00','120 mnt','Online','Rp 150.000','Selesai'],
        ['#PES-2026-0340','Pak Fauzan','B. Inggris','20 Mar 2026, 16:00','60 mnt','Online','Rp 70.000','Dibatalkan'],
    ];
    ...
    // Style lebar kolom
    ws['!cols'] = [
        {wch:18},{wch:22},{wch:16},{wch:22},{wch:10},{wch:10},{wch:12},{wch:16}
    ];
...
function exportPDF() {
    ...
    doc.text('Dicetak: ' + new Date().toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'}), 200, 22);

    doc.setTextColor(0,0,0);

    doc.autoTable({
        startY: 36,
        head: [['ID Pesanan','Tutor','Mata Pelajaran','Jadwal','Durasi','Mode','Biaya','Status']],
        body: [
            ['#PES-2026-0412','Pak Budi Santoso','Matematika','8 Apr 2026, 10:00','90 mnt','Online','Rp 75.000','Dikonfirmasi'],
            ['#PES-2026-0398','Bu Sari Dewi','Fisika','5 Apr 2026, 14:00','90 mnt','Offline','Rp 90.000','Selesai'],
            ['#PES-2026-0385','Pak Rizal Hakim','Kimia','2 Apr 2026, 09:00','60 mnt','Online','Rp 80.000','Selesai'],
...
function exportSinglePDF() {
    ...
    doc.autoTable({
        ...
        columnStyles:{
            0:{fontStyle:'bold',fillColor:[240,246,255],textColor:[30,58,95],cellWidth:55},
            1:{cellWidth:120}
        },
        theme:'plain',
    ...
    doc.setFontSize(8);
    doc.setTextColor(150);
    doc.text('Dicetak oleh Al Ilmi Center · ' + new Date().toLocaleDateString('id-ID'), 14, doc.internal.pageSize.height - 10);
