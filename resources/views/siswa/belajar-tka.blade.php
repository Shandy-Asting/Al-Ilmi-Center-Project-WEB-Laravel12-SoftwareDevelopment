...
@push('styles')
<style>
    .step-bar{display:flex;align-items:center;background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:14px 24px;margin-bottom:28px;overflow-x:auto;}
    .step-item{display:flex;align-items:center;gap:10px;flex-shrink:0;}
    .step-circle{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;flex-shrink:0;}
    ...
    .step-divider{flex:1;height:2px;background:var(--border);margin:0 12px;min-width:24px;}
    .step-divider.done{background:var(--success);}
    .jenjang-card{background:var(--card-bg);border:2px solid var(--border);border-radius:16px;padding:22px 18px;text-align:center;cursor:pointer;transition:all .2s;height:100%;}
    .jenjang-card:hover{border-color:var(--primary-light);transform:translateY(-3px);box-shadow:0 8px 24px rgba(30,58,95,.12);}
    .jenjang-card.selected{border-color:var(--primary);background:#eff6ff;box-shadow:0 6px 20px rgba(30,58,95,.18);}
    .jenjang-icon{font-size:36px;margin-bottom:10px;}
    .jenjang-title{font-size:15px;font-weight:800;margin-bottom:4px;color:var(--text);}
    .jenjang-sub{font-size:12px;color:var(--muted);}
    .jenjang-badge{display:inline-block;margin-top:8px;font-size:10.5px;font-weight:700;padding:3px 10px;border-radius:20px;}
    .materi-card{background:var(--card-bg);border:1.5px solid var(--border);border-radius:14px;padding:16px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:all .2s;margin-bottom:10px;}
    .materi-card:hover{border-color:var(--primary-light);box-shadow:0 4px 14px rgba(30,58,95,.1);}
    .materi-card.selected{border-color:var(--primary);background:#eff6ff;}
    .materi-icon{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;}
    .m-title{font-size:14px;font-weight:700;color:var(--text);}
    .m-sub{font-size:12px;color:var(--muted);}
    .m-tags{margin-top:5px;display:flex;gap:6px;flex-wrap:wrap;}
    .materi-tag{font-size:10.5px;font-weight:700;padding:2px 8px;border-radius:6px;}
    .materi-progress{margin-left:auto;text-align:center;min-width:56px;}
    .mp-val{font-size:16px;font-weight:800;color:var(--primary);}
    .mp-label{font-size:10px;color:var(--muted);}
    .tka-tabs{display:flex;gap:8px;margin-bottom:20px;background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:6px;overflow-x:auto;flex-wrap:wrap;}
    .tka-tab{flex-shrink:0;padding:9px 16px;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s;color:var(--muted);border:none;background:transparent;}
    .tka-tab.active{background:var(--primary);color:#fff;box-shadow:0 3px 10px rgba(30,58,95,.25);}
    .tka-tab:hover:not(.active){background:var(--bg);color:var(--primary);}
    .soal-header{background:linear-gradient(135deg,var(--primary) 0%,var(--primary-light) 60%,#3b6fa0 100%);border-radius:16px;padding:20px 24px;color:#fff;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;}
    .soal-header h5{font-size:16px;font-weight:800;margin-bottom:3px;}
    .soal-header p{font-size:12.5px;opacity:.8;margin:0;}
    .timer-box{background:rgba(255,255,255,.15);border-radius:10px;padding:8px 16px;text-align:center;}
    .t-val{font-size:22px;font-weight:800;}
    .t-label{font-size:10px;opacity:.7;}
    .soal-card{background:var(--card-bg);border:1px solid var(--border);border-radius:16px;padding:24px;}
    .soal-num{font-size:11px;font-weight:700;color:var(--primary);letter-spacing:.06em;text-transform:uppercase;margin-bottom:10px;}
    .soal-text{font-size:15px;line-height:1.7;margin-bottom:20px;font-weight:500;color:var(--text);}
    .opsi-item{display:flex;align-items:flex-start;gap:12px;padding:12px 16px;border:1.5px solid var(--border);border-radius:12px;margin-bottom:10px;cursor:pointer;transition:all .2s;}
    .opsi-item:hover{border-color:var(--primary-light);background:#f8faff;}
    .opsi-item.selected{border-color:var(--primary);background:#eff6ff;}
    .opsi-key{width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;background:var(--bg);color:var(--muted);flex-shrink:0;}
    .opsi-item.selected .opsi-key{background:var(--primary);color:#fff;}
    .opsi-text{font-size:13.5px;line-height:1.5;padding-top:4px;color:var(--text);}
    .soal-nav{display:flex;align-items:center;justify-content:space-between;margin-top:20px;}
    .btn-soal{border:none;border-radius:10px;padding:10px 22px;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s;}
    .btn-soal-prev{background:var(--bg);color:var(--muted);}
    .btn-soal-prev:hover{background:#eff6ff;color:var(--primary);}
    .btn-soal-next{background:var(--primary);color:#fff;box-shadow:0 4px 12px rgba(30,58,95,.3);}
    .btn-soal-submit{background:var(--success);color:#fff;box-shadow:0 4px 12px rgba(22,163,74,.3);}
    .nomor-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;}
    .nomor-btn{width:100%;aspect-ratio:1;border-radius:8px;border:1.5px solid var(--border);background:var(--bg);font-size:12px;font-weight:700;cursor:pointer;transition:all .2s;color:var(--muted);}
    .nomor-btn.answered{background:var(--primary);border-color:var(--primary);color:#fff;}
    .nomor-btn.current{background:var(--primary-light);border-color:var(--primary-light);color:#fff;box-shadow:0 0 0 3px rgba(30,58,95,.2);}
    .pembahasan-box{background:var(--success-soft);border:1.5px solid #a7f3d0;border-radius:14px;padding:18px 20px;margin-top:16px;}
    .pb-title{font-size:13px;font-weight:800;color:var(--success);margin-bottom:8px;display:flex;align-items:center;gap:8px;}
    .pb-text{font-size:13.5px;line-height:1.7;color:var(--text);}
    .hasil-banner{background:linear-gradient(135deg,var(--primary) 0%,var(--primary-light) 60%,#3b6fa0 100%);border-radius:20px;padding:30px 32px;color:#fff;text-align:center;margin-bottom:20px;}
    .hasil-score{font-size:64px;font-weight:800;line-height:1;}
    .hasil-score span{font-size:22px;}
    .hasil-grade{font-size:18px;font-weight:700;margin-top:6px;opacity:.9;}
    .hasil-sub{font-size:13px;opacity:.75;margin-top:4px;}
    .hasil-stat{display:flex;justify-content:center;gap:32px;margin-top:20px;}
    .hs-val{font-size:22px;font-weight:800;}
    .hs-label{font-size:11px;opacity:.7;}
    ...
    .card-box{background:var(--card-bg);border-radius:16px;border:1px solid var(--border);padding:20px;}
    /* Modal file viewer */
    .file-viewer-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center;}
    .file-viewer-box{background:#fff;border-radius:16px;width:90%;max-width:900px;height:85vh;display:flex;flex-direction:column;overflow:hidden;}
    .file-viewer-header{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;}
...
<div id="step-materi" style="display:none;">
    ...
    <div class="materi-card"
        ...
        data-soal="{{ $m->soal_count }}"
        data-judul="{{ $m->judul }}"
        data-mapel-label="{{ $m->mata_pelajaran }}"
        data-kelas="{{ $m->kelas }}"
        data-file="{{ $m->file_path ? asset('storage/'.$m->file_path) : '' }}"
        data-link="{{ $m->link_video ?? '' }}"
        data-tipe="{{ $m->tipe }}"
        ...
        <div class="flex-grow-1">
            ...
            <div class="m-sub">
                ...
                @if($m->tutor) · <i class="bi bi-person-fill"></i> {{ $m->tutor->name }} @endif
            </div>
            <div class="m-tags">
                ...
                @elseif($m->file_path)
                <span class="materi-tag" style="background:#fee2e2;color:#dc2626;">📄 {{ strtoupper($m->tipe) }}</span>
                @endif
                @if($m->soal_count === 0)
                <span class="materi-tag" style="background:var(--danger-soft);color:var(--danger);">⚠ Belum ada soal</span>
                @else
                <span class="materi-tag" style="background:var(--success-soft);color:var(--success);">✓ {{ $m->soal_count }} Soal</span>
                @endif
            </div>
        </div>
    ...
    @empty
    <div style="text-align:center;padding:40px;background:var(--card-bg);border-radius:14px;border:1px solid var(--border);">
    ...
    <div id="no-materi" style="display:none;text-align:center;padding:30px;background:var(--card-bg);border-radius:14px;border:1px solid var(--border);">
        ...
        <div style="background:#eff6ff;border:1.5px solid var(--primary);border-radius:14px;padding:16px 20px;">
            <div class="d-flex align-items-center gap-3 flex-wrap">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    ...
                    <button id="btn-buka-materi" style="display:none;background:var(--info-soft);color:var(--info);border-radius:10px;border:none;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer;"
...
<div id="fileViewerOverlay" class="file-viewer-overlay" style="display:none;" onclick="tutupFileViewer(event)">
    <div class="file-viewer-box" onclick="event.stopPropagation()">
        <div class="file-viewer-header">
            ...
            <button onclick="tutupFileViewer()" style="background:none;border:none;font-size:1.4rem;cursor:pointer;color:var(--muted);">
...
function lanjutKeMateri() {
    if (!jenjangDipilih) { alert('Pilih jenjang terlebih dahulu!'); return; }
    setStep(2);
    const lbl = { sd:'🏫 SD / Sederajat', smp:'🏛️ SMP / Sederajat', sma:'🎓 SMA / Sederajat' };
...
function pilihMateri(el, id, soalCount) {
    ...
    setTimeout(() => {
        document.getElementById('tombol-aksi').scrollIntoView({ behavior:'smooth', block:'center' });
    }, 150);
}
...
function mulaiBelajar(tipe) {
    if (!materiDipilih) { alert('Pilih materi terlebih dahulu!'); return; }
...
function setStep(step) {
    for (let i = 1; i <= 3; i++) {
        ...
        if (!c) continue;
        if (i < step) {
            c.className = 'step-circle done'; c.innerHTML = '<i class="bi bi-check-lg"></i>';
            l.className = 'step-label done';
            if (d) d.className = 'step-divider done';
        } else if (i === step) {
            c.className = 'step-circle active'; c.innerHTML = i;
            l.className = 'step-label active';
        } else {
            c.className = 'step-circle pending'; c.innerHTML = i;
