    ...
    <div class="main-tabs no-print">
        <button class="main-tab active" onclick="switchTab(this,'tagihan')">
            ...
            @if ($totalBelumBayar > 0)
                <span
                    style="background:var(--danger);color:#fff;font-size:10px;padding:1px 5px;border-radius:20px;margin-left:3px;">{{ $totalBelumBayar }}</span>
            @endif
        </button>
        ...
        @forelse($tagihan as $t)
            ...
            <div class="tagihan-card {{ $isUrgent ? 'urgent' : '' }}">
                <div class="th-header {{ $isUrgent ? 'urgent' : 'normal' }}">
                    <span
                        class="th-id">#{{ $t->pembayaranTerakhir?->nomor_invoice ?? 'INV-' . strtoupper(substr($t->id, 0, 8)) }}</span>
                    <div class="d-flex align-items-center gap-2">
                        @if ($isUrgent && !$sudahUpload)
                            <span
                                style="background:var(--danger-soft);color:var(--danger);font-size:10.5px;font-weight:700;padding:3px 8px;border-radius:20px;">
                ...
                <div class="th-body">
                    <div class="th-row">
                        ...
                        <div class="th-info">
                            <div class="th-title">Les Privat –
                                {{ $t->mata_pelajaran }}{{ $t->topik ? ' – ' . $t->topik : '' }}</div>
                            <div class="th-sub"><i class="bi bi-person-fill"></i> {{ $t->tutor->name ?? '-' }}</div>
                            <div class="th-sub"><i class="bi bi-calendar3"></i>
                                {{ $t->jadwal->translatedFormat('l, d M Y · H:i') }} WIB</div>
                            <div class="th-sub"><i class="bi bi-clock"></i> {{ $t->durasi_menit }} mnt ·
                                {{ $t->getModeLabel() }}</div>
                            <div class="th-sub"
                        ...
                        <div class="th-amount">
                            ...
                            <div style="font-size:11px;color:var(--muted);">
                                {{ $sudahUpload ? 'Menunggu Verifikasi' : 'Belum Lunas' }}</div>
                        </div>
                    </div>
                </div>
                <div class="th-footer">
        ...
        @empty
            <div
                ...
                <i class="bi bi-check-circle"
                    style="font-size:2.5rem;display:block;margin-bottom:10px;color:var(--success);"></i>
                ...
                <table class="tbl-riwayat">
                    ...
                    <tbody>
                        @forelse($riwayat as $r)
                            <tr>
                                <td style="font-weight:700;font-size:12px;color:var(--primary);">#{{ $r->nomor_invoice }}
                                </td>
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" style="text-align:center;padding:32px;color:var(--muted);">Belum ada
    ...
    <div class="modal-overlay" id="modal-bayar">
        <div class="modal-box">
            ...
            <div style="padding:20px 22px;">
                ...
                {{-- Info Tagihan --}}
                <div
                    style="background:var(--bg);border-radius:12px;padding:14px 16px;margin-bottom:20px;border-left:4px solid var(--primary);">
                    <div
                        style="font-size:11px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">
                        Detail Tagihan</div>
                    ...
                    <div style="font-size:20px;font-weight:800;color:var(--primary);margin-top:8px;" id="modal-jumlah">-
                    </div>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                    <div
                        style="width:24px;height:24px;border-radius:50%;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;">
                        1</div>
                ...
                {{-- Instruksi --}}
                <div
                    style="background:var(--accent-soft);border:1px solid var(--accent);border-radius:10px;padding:12px 14px;margin:16px 0;font-size:12.5px;color:#92400e;">
                    <div style="font-weight:700;margin-bottom:6px;"><i class="bi bi-info-circle-fill me-1"></i>Cara
                        Pembayaran:</div>
                ...
                <div id="previewWrap" style="display:none;margin-top:10px;text-align:center;">
                    <img id="previewImg"
                        style="max-width:100%;max-height:180px;border-radius:10px;border:2px solid var(--success);" />
                ...
                <div class="d-flex gap-2 mt-4">
                    ...
                    <button type="button" class="btn fw-bold flex-fill py-2" onclick="kirimPembayaran()"
    ...
    <div class="modal-overlay" id="modal-sukses">
        <div class="modal-box" style="max-width:420px;">
            <div style="padding:32px 28px;text-align:center;">
                ...
                <div
                    style="background:var(--success-soft);border-radius:12px;padding:14px;margin-bottom:20px;text-align:left;">
    ...
    <script>
        ...
        function openModalBayar(id, layanan, jumlah, tutor) {
            ...
            document.getElementById('uploadText').innerHTML =
                '<div style="font-size:13.5px;font-weight:700;color:var(--primary);">Klik untuk pilih foto</div><div style="font-size:12px;color:var(--muted);margin-top:4px;">JPG, PNG — Maks. 2MB</div>';
        ...
        function previewUpload(input) {
            ...
            reader.onload = e => {
                ...
                document.getElementById('uploadText').innerHTML =
                    '<div style="font-size:13px;font-weight:700;color:var(--success);">File siap dikirim</div><div style="font-size:11.5px;color:var(--muted);margin-top:3px;">Klik untuk ganti foto</div>';
            };
