...
@push('styles')
<style>
    .hero-banner{background:linear-gradient(135deg,var(--primary) 0%,var(--primary-light) 60%,#3b6fa0 100%);border-radius:16px;padding:28px 32px;display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;position:relative;overflow:hidden;}
    .hero-banner::before{content:'';position:absolute;top:-40px;right:120px;width:200px;height:200px;background:rgba(255,255,255,.05);border-radius:50%;}
    .hero-banner::after{content:'';position:absolute;bottom:-60px;right:60px;width:280px;height:280px;background:rgba(255,255,255,.04);border-radius:50%;}
    .hero-text .greeting{color:rgba(255,255,255,.7);font-size:13px;font-weight:500;}
    .hero-text .title{color:#fff;font-size:22px;font-weight:800;margin-top:2px;}
    .hero-text .sub{color:rgba(255,255,255,.6);font-size:13px;margin-top:6px;max-width:420px;}
    .hero-pills{display:flex;gap:8px;margin-top:16px;flex-wrap:wrap;}
    .hero-pill{background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.2);color:#fff;font-size:12px;font-weight:600;padding:5px 12px;border-radius:20px;display:flex;align-items:center;gap:5px;}
    .hero-pill.accent{background:var(--accent);color:var(--primary);border-color:var(--accent);}
    .hero-stats{display:flex;gap:24px;position:relative;z-index:1;flex-shrink:0;}
    .hero-stat-item{text-align:center;}
    .hero-stat-num{font-size:28px;font-weight:800;color:#fff;line-height:1;}
    .hero-stat-lbl{font-size:11px;color:rgba(255,255,255,.6);margin-top:4px;font-weight:500;}
    .stat-card{background:var(--card-bg);border-radius:14px;padding:20px;border:1px solid var(--border);transition:all .2s;}
    ...
    .stat-value{font-size:26px;font-weight:800;color:var(--text);margin-top:12px;line-height:1;}
    .stat-label{font-size:12.5px;color:var(--muted);font-weight:500;margin-top:4px;}
    .stat-bar-track{height:5px;background:#f1f5f9;border-radius:99px;overflow:hidden;margin-top:12px;}
    .stat-bar-fill{height:100%;border-radius:99px;}
    .stat-bar-meta{display:flex;justify-content:space-between;font-size:11px;color:var(--muted);margin-top:5px;}
    ...
    .bc-bar{width:100%;border-radius:5px 5px 0 0;transition:height .4s ease;position:relative;}
    .bc-label{font-size:10px;color:var(--muted);font-weight:500;}
    .bc-primary{background:linear-gradient(180deg,var(--primary-light),var(--primary));}
    .bc-accent{background:linear-gradient(180deg,#fbbf24,var(--accent));}
    .bc-success{background:linear-gradient(180deg,#34d399,var(--success));}
    .tbl{width:100%;border-collapse:collapse;}
    .tbl thead th{background:#f8fafc;font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.6px;padding:10px 14px;border-bottom:1px solid var(--border);white-space:nowrap;}
    ...
    .p-primary{background:#eff6ff;color:var(--primary);}
    .feed-item{display:flex;gap:12px;padding:12px 18px;border-bottom:1px solid var(--border);align-items:flex-start;}
    .feed-item:last-child{border-bottom:none;}
    .feed-icon{width:36px;height:36px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:15px;}
    .status-row{display:flex;align-items:center;padding:10px 18px;border-bottom:1px solid var(--border);font-size:13px;}
    .status-row:last-child{border-bottom:none;}
    .status-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
    .dot-green{background:var(--success);}
    .dot-yellow{background:var(--warning);}
    .qa-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:16px;}
    .qa-btn{display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:10px;border:1px solid var(--border);background:#fff;cursor:pointer;transition:all .2s;text-decoration:none;}
    .qa-btn:hover{border-color:var(--primary);background:#eff6ff;}
    .qa-icon{width:34px;height:34px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:15px;transition:all .2s;}
    .qa-btn:hover .qa-icon{background:var(--primary);color:#fff;}
    .qa-label{font-size:12.5px;font-weight:600;color:var(--text);}
    .qa-sub{font-size:11px;color:var(--muted);margin-top:1px;}
    @media(max-width:767px){.hero-stats{display:none;}.hero-banner{padding:18px 20px;}.stat-value{font-size:20px;}.bar-chart{height:100px;}.qa-grid{grid-template-columns:1fr;}}
</style>
@endpush
...
<div class="hero-banner mb-4">
    <div class="hero-text">
        ...
        <div class="sub">Semua berjalan normal. Ada 5 pembayaran pending & 3 pesanan les baru masuk.</div>
        <div class="hero-pills">
            <div class="hero-pill accent"><i class="bi bi-lightning-charge-fill"></i> 5 Pembayaran Pending</div>
            <div class="hero-pill"><i class="bi bi-person-video3"></i> 3 Pesanan Les Baru</div>
    ...
    <div class="hero-stats">
        ...
        $totalLes   = \App\Models\LesPrivat::count();
        @endphp
...
<div class="row g-3 mb-4">
    @php
    $cards = [
        ['icon'=>'bi-people-fill','bg'=>'#eff6ff','color'=>'var(--primary)','val'=>\App\Models\User::count(),'label'=>'Total Pengguna','bar'=>'82%','bar_color'=>'var(--primary)','trend'=>'+18%','trend_up'=>true],
        ['icon'=>'bi-person-check-fill','bg'=>'var(--success-soft)','color'=>'var(--success)','val'=>\App\Models\User::where('role','siswa')->count(),'label'=>'Total Siswa','bar'=>'68%','bar_color'=>'var(--success)','trend'=>'+12%','trend_up'=>true],
        ['icon'=>'bi-cash-stack','bg'=>'var(--accent-soft)','color'=>'var(--warning)','val'=>'Rp '.number_format(\App\Models\LesPrivat::where('status','selesai')->sum('harga')/1000000,1).'Jt','label'=>'Pendapatan Les','bar'=>'73%','bar_color'=>'var(--accent)','trend'=>'+15%','trend_up'=>true],
        ['icon'=>'bi-book-fill','bg'=>'var(--info-soft)','color'=>'var(--info)','val'=>\App\Models\LesPrivat::where('status','selesai')->count(),'label'=>'Sesi Selesai','bar'=>'90%','bar_color'=>'var(--info)','trend'=>'+31%','trend_up'=>true],
    ];
    @endphp
    @foreach($cards as $c)
        ...
        <div class="card-box">
            ...
            <div class="p-4">
                <div class="bar-chart">
                    ...
                    <div class="bc-col"><div class="bc-bar bc-primary" style="height:{{ $siswaH[$i] }}%"></div><div class="bc-label">{{ $m }}</div></div>
                    @endforeach
                    <div style="width:1px;background:var(--border);margin:0 8px;"></div>
                    @foreach($months as $i => $m)
                    <div class="bc-col"><div class="bc-bar bc-accent" style="height:{{ $tutorH[$i] }}%"></div><div class="bc-label">{{ $m }}</div></div>
                    @endforeach
                    <div style="width:1px;background:var(--border);margin:0 8px;"></div>
                    @foreach($months as $i => $m)
                    <div class="bc-col"><div class="bc-bar bc-success" style="height:{{ $lesH[$i] }}%"></div><div class="bc-label">{{ $m }}</div></div>
                    @endforeach
                </div>
...
<div class="row g-3 mb-3">
    ...
    <div class="col-12 col-xl-8">
        <div class="card-box">
            ...
            @php
            $feeds = [
                ['bg'=>'#eff6ff','color'=>'var(--primary)','icon'=>'bi-person-plus-fill','title'=>'Siswa baru mendaftar','desc'=>'Fajar Nugroho bergabung sebagai siswa SMA','time'=>'5 mnt'],
                ['bg'=>'var(--success-soft)','color'=>'var(--success)','icon'=>'bi-credit-card-fill','title'=>'Pembayaran dikonfirmasi','desc'=>'Pak Budi mengkonfirmasi pembayaran Andi Rp 75.000','time'=>'12 mnt'],
                ['bg'=>'var(--accent-soft)','color'=>'var(--warning)','icon'=>'bi-person-badge-fill','title'=>'Tutor baru terdaftar','desc'=>'Dina Amalia mendaftar sebagai tutor Kimia','time'=>'28 mnt'],
                ['bg'=>'var(--warning-soft)','color'=>'var(--warning)','icon'=>'bi-exclamation-triangle-fill','title'=>'Storage mendekati kapasitas','desc'=>'Penggunaan storage mencapai 84% dari kapasitas','time'=>'1 jam'],
                ['bg'=>'var(--danger-soft)','color'=>'var(--danger)','icon'=>'bi-x-circle-fill','title'=>'Pesanan les dibatalkan','desc'=>'Siti Rahma membatalkan sesi Matematika','time'=>'2 jam'],
                ['bg'=>'var(--info-soft)','color'=>'var(--info)','icon'=>'bi-upload','title'=>'Materi baru diunggah','desc'=>'Tutor Budi — Matematika SMA Bab 7','time'=>'3 jam'],
            ];
            @endphp
            @foreach($feeds as $f)
            <div class="feed-item">
    ...
    <div class="col-12 col-xl-4">
        <div class="card-box">
            <div class="card-box-header"><div class="card-box-title">Aksi Cepat</div></div>
            <div class="qa-grid">
                @php
                $qaItems = [
                ...
                <a href="{{ $q[0] }}" class="qa-btn">
                    ...
                    <div><div class="qa-label">{{ $q[4] }}</div><div class="qa-sub">{{ $q[5] }}</div></div>
                </a>
                @endforeach
            </div>
        </div>
...
<div class="card-box">
    ...
    <div style="overflow-x:auto;">
        <table class="tbl">
            <thead>
                <tr>
                    <th>Pengguna</th><th>Peran</th><th>Tanggal Daftar</th><th>Status</th><th>Aksi</th>
                </tr>
            </thead>
            <tbody>
            ...
            @empty
            <tr><td colspan="5" style="text-align:center;padding:32px;color:var(--muted);">Belum ada pengguna</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
